...
## Session 55: The Workshops Were Indexed All Along — Their Content Was Being Thrown Away

*2026-09-20 · 10:40-12:00*

**Tracks:** corpus-ingestion, vector-architecture, worker-api, operations

**Session costs:** deck re-extraction (vision): $0.17

**Vectors upserted:** total: 33,778 · symposium: 371

- **The programme now answers programme questions.** Queried at equal weight alongside ten other namespaces, the symposium lost races it should win: *"What workshops are happening at the Protocol Symposium?"* ranked a **2025** Substack post above the programme and surfaced 2 of the 5 workshops. A question that names the event is now answered from the symposium namespace alone, with the other namespaces skipped rather than fetched and discarded — which also spends less Pinecone egress.Three limits keep the rule honest. The trigger is the event being named, not any word the programme happens to contain, so *"what did the MRG say about memory?"* is untouched. A year other than 2026 cancels scoping — without that, *"summarize the 2025 symposium"* would have been answered confidently from the wrong event. And an explicit reach for prior work keeps the whole corpus in play, because connecting a talk to the archive is the thing the programme page cannot do.Retrieval is still never filtered by *date*. A talk that has already been delivered is perfectly good material; time-awareness shapes recommendations, not what can be found.

- Scoping on the word alone left the questions people actually type during a conference unscoped. *"Which sessions run on September 23?"* returned 3 of 8 sources from the programme — and three of the rest from C3PO's own build log.A programme word plus a temporal cue aimed at the event now counts as naming it: an explicit event date at any time, or a relative day (today, tomorrow, this week) *only while the event is running*. Outside that window "today" says nothing about the symposium, and a programme word with no temporal cue — *"what sessions has the MRG held?"* — stays unscoped even mid-event.

- **The workshops were indexed the whole time. What was missing was their content.** Each chunk stores the text the model reads separately from the text it was embedded from, and the stored copy was capped at 1,600 characters. So a workshop was retrieved correctly and then described without its takeaways or activities — which, from the outside, is indistinguishable from not being in the index at all.Measured before the fix: **13,124 characters** dropped across the programme, cutting 26 of 61 session chunks and 3 of 5 workshop details mid-sentence. The two richest workshops lost more than half their detail. The slide decks were worse — the chunker targets about 2,300 characters, so **37 of 40 sampled deck chunks sat exactly at the cap**, meaning roughly a third of every deck never reached the model. The cap is now 6,000 with a paragraph-boundary trim and a warning above it.The general shape is worth naming: a limit that only affects what a model *sees*, never what it *finds*, produces a system that retrieves the right document and then talks about it badly. Nothing in retrieval metrics would show it.

- Three entries in the symposium's Drive folder were not documents. A Drive **shortcut** pointed at another file in the same folder that was already ingested — the right response being not to ingest it twice. A **zip** turned out to be a bundle: two slide variants, an HTML export, a framework document and 46KB of speaker notes. And a deck logged for days as an empty stub, "speaker has not filled it in", had exactly one slide reading *"Link to slides: …"* — the deck was never missing, it was hosted elsewhere. One hop retrieves 14,411 characters of it.Pointers are followed exactly one hop and only from documents under 400 characters. In a real deck a URL is a citation, and chasing citations would walk the ingest into the open web.

- The AI Kitcraft material existed in seven near-identical forms at 0.70–0.91 token overlap. Ingesting them all would have put roughly 230,000 characters of a single workshop into a namespace where other talks hold two to six chunks each, and duplicate chunks compete for the same retrieval slots.Files that land on the same talk now collapse to the richest, with the rest recorded and the verdict remembered so a settled group is not re-downloaded every cycle. The threshold is set deliberately low enough to preserve genuinely different documents filed under one talk: a deck and its companion paper score below it and both survive.

- The devlog page renders into a single SQL statement, and D1 caps a statement at 100KB — so the renderer had been truncating from the front. **22 of 56 sessions were missing from the published page.**The row itself can hold 2MB, so the page is now written as one insert plus a series of appends, each sized against a byte budget, and read back and compared before the write is recorded as successful. The log is also paged now — older entries roll into archive files — but that split is invisible: every consumer loads the merged record, so the page, the Markdown rendering and the vector index all still see one continuous history with working anchors.
