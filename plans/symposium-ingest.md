...
**Goal (VGR, session 54):** through the symposium, c3po should be able to
(1) answer detail questions about the program, (2) recommend particular talks,
and (3) converse intelligently about the themes of the workshops.
...
| `symposium_block` | special session (4) + each program day (3) | what runs in the block, in order — the countable fact, so "what's on Thursday" is not inferred from retrieved fragments |
...
Retrieval alone will not deliver what VGR asked for. Three distinct mechanisms:

### 1. "What's on now / next / tomorrow" — not a retrieval question

Times are UTC in the data; the answer depends on wall-clock time at query
moment. A nearest-neighbour search over abstracts cannot compute it and will
confabulate plausibly.

**Proposal:** a compact **schedule digest** — the full 61-item grid as ~2KB of
structured text — injected into the worker's system prompt during the event
window, the way the 40-term protocol lexicon already is
(`feedback_vgr_zirp_pattern`: the lexicon IS injected into the system prompt).
Regenerated by the Phase A sync, stored in KV, injected when the query looks
symposium-shaped or unconditionally between Sept 20 and Sept 26. The model then
has the whole grid in context and can do the time arithmetic, rather than
reasoning from five retrieved chunks.

Needs the current time in the prompt too — worth confirming the worker already
passes one.

### 2. "Recommend a talk" — retrieval plus cross-corpus grounding

Query `symposium` with the asker's stated interest, then cross-reference the
existing namespaces so the recommendation can say *why*: "Provenance Against
Fluency, Thursday 20:30 UTC — connects to the MRG thread on archival
authenticity from August." That grounding is the thing c3po can do that the
program page cannot, and it is the argument for a dedicated namespace queried
alongside the others rather than a standalone lookup tool.

Guard against the intro-handler failure mode (`feedback_intro_handler`): the
recommendation logic selects *which* talks to name; it must not be allowed to
suppress or distort the generated answer.

### 3. "Themes of the workshops" — the `symposium_workshop` chunk
...
`symposium_block` and `symposium_overview` are few and will lose a
nearest-neighbour race against 61 rich abstracts. Fire a parallel filtered
sub-query for them, the way `sig_meeting_page` already does, so "how many talks
are there" and "when does it start" don't depend on getting lucky at `TOP_K_EACH`.
...
2. **Unscheduled Art of Memory talks.** Five MRG talks sit in a block whose
   order the host sets live. Answer as "sometime in the 19:00–21:30 block on
   Friday" (accurate, vague) or chase the running order before the event?
3. **Phase B trigger date.** Proposed: a first pass Sept 19–20 once decks firm
   up, a second after the event captures finals.
4. **Does the bot say anything about registration?** It is closed at capacity,
